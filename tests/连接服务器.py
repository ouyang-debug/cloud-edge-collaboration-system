...
def execute_command(client, command, server_info):
    """
    执行SSH命令
    :param client: SSH客户端对象
    :param command: 要执行的命令
    :param server_info: 服务器信息，格式为 "IP:端口"
    :return: 命令执行结果
    """
    try:
        # 显示正在执行命令的服务器信息
        print(f"\n=== 在服务器 {server_info} 上执行命令 ===")
...
def view_file(client, file_path, server_info):
    """
    查看文件内容
    :param client: SSH客户端对象
    ...
    try:
        ...
        content = execute_command(client, command, server_info)
        return content
    except Exception as e:
...
def get_latest_folder(client, directory, server_info):
    """
    获取目录下最新日期的文件夹
    :param client: SSH客户端对象
    ...
    try:
        # 执行ls -la命令查看目录内容
        ls_output = execute_command(client, f"ls -la {directory}", server_info)
        if not ls_output:
            return None
        ...
        try:
            while True:
                ...
                if choice == '1':
                    ...
                    # 首先执行ls -la命令查看文件信息
                    server_info = f"{hostname}:{port}"
                    tasks_json_ls = execute_command(client, f"ls -la {tasks_json_path}", server_info)
                    print(tasks_json_ls)
                    # 然后查看文件内容
                    tasks_json_content = view_file(client, tasks_json_path, server_info)
                    ...
                    server_info = f"{hostname}:{port}"
                    tasks_dir_ls = execute_command(client, f"ls -la {tasks_dir_path}", server_info)
                    print(tasks_dir_ls)

                    # 分析目录内容，找到最新日期的文件夹
                    print("\n=== 分析目录内容，找到最新日期的文件夹 ===")
                    latest_folder = get_latest_folder(client, tasks_dir_path, server_info)
                    if latest_folder:
                        ...
                        if view_latest == 'y':
                            ...
                            # 首先执行ls -la命令查看文件信息
                            latest_file_ls = execute_command(client, f"ls -la {latest_file_path}", server_info)
                            print(latest_file_ls)
                            # 然后查看文件内容
                            latest_file_content = view_file(client, latest_file_path, server_info)
                    ...
                    # 首先执行ls -la命令查看文件信息
                    server_info = f"{hostname}:{port}"
                    new_path_ls = execute_command(client, f"ls -la {new_path}", server_info)
                    print(new_path_ls)
                    # 然后查看文件内容
                    new_path_content = view_file(client, new_path, server_info)
                    if new_path_content:
                        # 美化打印JSON内容
                    ...
                    print("\n=== 执行ls -la命令查看文件信息 ===")
                    server_info = f"{hostname}:{port}"
                    ls_output = execute_command(client, f"ls -la {file_path}", server_info)
                    print(ls_output)

                    print("\n=== 查看文件内容 ===")
                    file_content = view_file(client, file_path, server_info)
                    if file_content:
                        # 美化打印JSON内容
